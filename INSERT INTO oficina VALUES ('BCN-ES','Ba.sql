...
-----------(2)-----------
Select C.codigo_cliente,O.linea_direccion1, O.linea_direccion2
From Oficina O
...
Select C.nombre_cliente
From cliente C
Where C.codigo_cliente not in (Select P.codigo_cliente From pedido P) and C.codigo_cliente not in (Select P.codigo_cliente From Pago P);
...
Select E.nombre
From empleado E
Where E.codigo_empleado not in (Select codigo_empleado_rep_ventas From Cliente) and E.codigo_oficina is Null;
...
-----------(20)-----------
Select count(P.codigo_pedido)as Cant_Pedido, P.estado
From pedido P
group by P.estado
...
--Nombre de  Clientes que han realizado pedidos de productos de la gama 'Herramientas':
...
--Empleados que tienen un salario superior al salario promedio de la compañía:
...
From Oficina O
inner join empleado E on O.codigo_oficina = E.codigo_oficina
Where E.puesto = 'Secretaria';

Select *
From Oficina O
Where O.codigo_oficina in (Select E.codigo_oficina From Empleado E Where E.puesto = 'Secretaria');

Select *
From producto P
Where P.codigo_producto not in (Select DE.codigo_producto From detalle_pedido DE );